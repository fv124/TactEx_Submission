...
def structure_input_LLM(centroids, phrases, HA):
    if HA == None:
        HA = np.random.randint(0, 45, size=len(phrases))   
...
def get_LLM_response(objects_structured):
    ...
    - Bananas: ripe if hardness is between 15 and 25 HA.
...
### test above logic
centroids = [(100, 200), (150, 250), (200, 300)]
phrases = ["banana", "banana", "banana"]
output = structure_input_LLM(centroids, phrases, None)

response = get_LLM_response(output)
print(response)

def evaluate_response_with_groq(scene_input_json: str, model_response: str) -> str:
    API_KEY = "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"  # your Groq key
    headers = {
        "Authorization": f"Bearer {API_KEY}",
        "Content-Type": "application/json"
    }
    ...
    judge_prompt = f"""
You are an expert evaluator of AI-generated scene descriptions. Your job is to rate the following response according to:

- **Conciseness** (succinct and avoids repetition)
- **Completeness** (includes all fruits/vegetables and their properties)
- **Correctness** (follows the rules about location and ripeness from input)

Rate each on a scale from 1–5 with a short explanation. Then say if this response is acceptable for user-facing output.
...
Conciseness: X/5 - explanation  
Completeness: X/5 - explanation  
Correctness: X/5 - explanation  
...
"""
    ...
    data = {
        "model": "llama-3.3-70b-versatile",
        "messages": [
            {"role": "system", "content": "You are a critical evaluator of scene descriptions generated by language models."},
