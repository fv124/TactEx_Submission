...
from Pretrain_Finetuning_Transformer.Pretraining_Online_Data_Transformer import Hardness_Transformer
...
if __name__ == "__main__":
    ...
    if leave_one_out == True:
        ...
        for obj in list(reversed(unique_objects))[:7]:
            ...
            model.load_state_dict(torch.load(f"Transformer.pth"))
            optimizer = AdamW([
            ...
            pred_df.to_csv(f"Transformer_Finetuned_{obj.replace(' ', '_')}_results.csv", index=False)
        ...
        results_df.to_csv(f"Transformer_Finetuned_results.csv", index=False)
    ...
    for epoch in range(15):
        ...
        if epoch_loss < best_loss:
            ...
            patience_counter = 0
            torch.save(model.state_dict(), f"Transformer_{pretrain_range}_Finetuned.pth")
        else:
            patience_counter += 1
